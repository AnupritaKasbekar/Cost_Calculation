...
function Merchandise_in_out() {
    const [listM_in_out,setM_in_out] = useState([]);
    const navigate = useNavigate();
    useEffect(()=>{
        axios.get("http://localhost:3001/mer_mat_inout_report").then((response)=>{
            setM_in_out(response.data)
        })
    },[])
   console.log("in Merchandise_in_out");
   let sam = 10.6
    console.log(sam.toFixed(3));
    const tablerows = listM_in_out.map((m_i_out)=>{
       return(
        ...
        <td  className='' key={m_i_out}> {m_i_out.code_id}</td>
          ...
          <td  className=''> {m_i_out.out_date}</td>
...
export default Merchandise_in_out
