...
function Raw_material_inOut() {
    const [listM_in_out,setM_in_out] = useState([]);
    const navigate = useNavigate();
    useEffect(()=>{
        axios.get("http://localhost:3001/raw_mat_inout_report").then((response)=>{
    ...
    const tablerows = listM_in_out.map((m_i_out)=>{
       return(
        ...
        <td  className='' key={m_i_out}> {m_i_out.code_id}</td>
          ...
          <td  className='textright'> {m_i_out.in_date}</td>
          
          <td  className='textright'> {m_i_out.remaining_qty}</td>
          
          <td  className='textright'> {m_i_out.sum_in_qty.toLocaleString("ko-KR",{minimumFractionDigits:3})}</td>
          <td  className='textright'> {m_i_out.div_in_price.toLocaleString("ko-KR",{minimumFractionDigits:2})}</td>
          
           <td  className='textright'> {m_i_out.sum_in_qty_price_into_cost.toLocaleString("ko-KR",{ minimumFractionDigits:0})}</td>
           <td  className='textright'> {m_i_out.sum_out_qty.toLocaleString("ko-KR",{minimumFractionDigits:3})}</td>
          <td  className='textright'> {m_i_out.div_out_price.toLocaleString("ko-KR",{ minimumFractionDigits:2})}</td>
          
           <td  className='textright'> {m_i_out.sum_out_qty_price_into_cost.toLocaleString("ko-KR",{minimumFractionDigits:0})}</td>
...
export default Raw_material_inOut
